...
- (void) awakeFromNib
{
  ...
  //NSLog(@"awake:start");
	...
	OptionAString=[[NSString string]retain];
	OptionBString=[[NSString string]retain];
	AdminProjektArray=[[NSMutableArray alloc]initWithCapacity:0];
	[AdminProjektArray retain];

	
	RPExportdaten=[[[NSUserDefaults standardUserDefaults] objectForKey:RPExportdatenKey]retain];
	//NSLog(@"awake: RPExportdaten; %d",[RPExportdaten description]);
	//NSLog(@"awake: RPExportdaten; %d",[RPExportdaten length]);
	ExportFormatString=(NSMutableString*)[[[NSUserDefaults standardUserDefaults] stringForKey:RPExportformatKey]retain];
	...
	//[  setToolTip:NSLocalizedString(@"",@"")];
	//[[[NamenListe tableColumnWithIdentifier:@"anz"]headerCell]contentView setToolTip:NSLocalizedString(@"Number of records of the reader",@"Anzahl Aufnahmen des Lesers")];
	...
	if (AdminLeseboxHit==NSOKButton)
	  {
		...
		tempLeseboxPfad=[[[AdminLeseboxDialog URL]path ]retain]; //"home"
		//tempLeseboxURL = [[AdminLeseboxDialog URL]retain]; 
...
- (void) setAdminPlayer:(NSString*)derLeseboxPfad inProjekt:(NSString*)dasProjekt
{
	...
	AdminLeseboxPfad=[[NSString stringWithString:derLeseboxPfad]retain];
	AdminArchivPfad=[[NSString stringWithString:[derLeseboxPfad stringByAppendingPathComponent:@"Archiv"]]retain];
	
	
	AdminProjektPfad=[[AdminArchivPfad stringByAppendingPathComponent:dasProjekt]retain];//Pfad des Archiv-Ordners
	[AdminProjektPfad retain];
	...
	tempAnzAufnahmenListe=[[[NSMutableArray alloc]initWithCapacity:0]autorelease];
	...
	for (i=0; i < [AdminProjektNamenArray count]; i++)
	{
		...
		AufnahmeFilesArray=[[NSMutableArray arrayWithCapacity:tempAnzAufnahmen]retain];
		...
		NSMutableArray* tempMarkArray=[[[NSMutableArray alloc]initWithCapacity:tempAnzAufnahmen]autorelease];
		int kk;
		...
		//tempPopUpButtonCell =[[NamenListe tableColumnWithIdentifier: @"aufnahmen"]dataCellForRow:i];
	...
	//[[NamenListe tableColumnWithIdentifier: @"neu"] setDataCell: PlayNeu];
	//for (i=0; i < AnzLeser; i++)
	//{
	//	[[[NamenListe tableColumnWithIdentifier: @"neu"]dataCellForRow:i]setTag:i];
	//}
...
{	
	...
	switch ([[[AufnahmenTab selectedTabViewItem]identifier]intValue])
	{
		case 1://Alle Aufnahmen
		{
			...
			//NSLog(@"AufnahmeInPlayer	clickedRow: %d",[NamenListe numberOfSelectedRows]);
			// 8.12.08
...
- (void)MarkierungEntfernenFuerZeile:(int)dieZeile
{
	...
	int tempAnzahlAufnahmen=[[tempZeilenDic objectForKey:@"anz"]intValue];
		...
		//int x=[[[NamenListe tableColumnWithIdentifier:@"aufnahmen"]dataCellForRow:tempZeile]indexOfSelectedItem];
...
- (void)alertDidEnd:(NSAlert *)alert returnCode:(int)returnCode contextInfo:(void *)contextInfo 
{
	...
	NSFileManager *Filemanager=[NSFileManager defaultManager];
	if ([(NSString*)contextInfo isEqualToString:@"TextchangedWarnung"])
	  {
		switch (returnCode)
		  {
	...
	if ([(NSString*)contextInfo isEqualToString:@"Entfernen"])
	  {
		switch (returnCode)
		  {
	...
	if ([(NSString*)contextInfo isEqualToString:@"keineLeser"])
	  {
	...
	if ([Quelle isEqualToString:@"AdminView"])
	  {
		...
		if ((nextZeilenNr>=0)&&AnzahlAufnahmenFuerZeile)
		  {
			[PlayTaste setEnabled:YES];
			//erfolg=[AdminFenster makeFirstResponder:PlayTaste];
			...
			if ([[AdminDaten AufnahmeFilesFuerZeile:selektierteZeile]count])
			  {
				...
				[self setBackTaste:YES];
				erfolg=[AdminFenster makeFirstResponder:zurListeTaste];
				//erfolg=[[self window]makeFirstResponder:PlayTaste];
				[PlayTaste setEnabled:NO];
				[MarkCheckbox setEnabled:YES];
				[self setLeserFuerZeile:selektierteZeile];
				NSLog(@"2");
...
- (void)MarkierungNotificationAktion:(NSNotification*)note
{
	...
	switch (var)
	{
		case 0://Nur Leser
		{
			...
			[self MarkierungEntfernenFuerZeile:[NamenListe selectedRow]];
