...
- (void)setAufnahmenVonLeser:(NSString*)derLeser
{
	...
	NSMutableArray* tempAufnahmenDicArray=[[[NSMutableArray alloc]initWithCapacity:0]autorelease];
	...
	while (eineAufnahme=[AufnahmenEnum nextObject])
	{
		...
		NSMutableDictionary* tempAufnahmenDic=[[[NSMutableDictionary alloc]initWithCapacity:0]autorelease];
...
//NSLog(@"objectValueForTableColumn");
    NSMutableDictionary *einAufnahmenDic=[[[NSMutableDictionary alloc]initWithCapacity:0]autorelease];
...
- (BOOL)tabView:(NSTabView *)tabView shouldSelectTabViewItem:(NSTabViewItem *)tabViewItem
{
	...
	if ([[tabViewItem identifier]intValue]==2)//zu 'Nach Namen'
	{
		...
		if ([NamenListe numberOfSelectedRows])//es ist eine zeile in der Namenliste selektiert
		{
			...
			int  Zeile;
			Zeile=[NamenListe selectedRow];//selektierte Zeile in der Namenliste
