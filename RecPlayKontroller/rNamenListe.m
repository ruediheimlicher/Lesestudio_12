...
- (id) init
{
	...
	{
		NamenArray=[[[NSMutableArray alloc] initWithCapacity: 0]autorelease];
	  neueNamenArray=[[[NSMutableArray alloc] initWithCapacity: 0]autorelease];
	  [neueNamenArray retain];
	}
	[NamenArray retain];
...
- (void) awakeFromNib
{
	...
	[ImportTaste setToolTip:NSLocalizedString(@"Look for list of names in finder",@"Namenliste im Finder suchen")];
...
- (IBAction)reportNameInListe:(id)sender
{
	...
	NSSortDescriptor* namenSort=[[[NSSortDescriptor alloc]initWithKey:@"namen" ascending:YES]autorelease];
