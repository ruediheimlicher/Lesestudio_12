...
*/
  ...
  BOOL istOrdner;
  NSFileManager *Filemanager = [NSFileManager defaultManager];
  NSString* HomeLeseboxPfad=[[NSHomeDirectory() stringByAppendingFormat:@"%@%@",@"/Documents",@"/Lesebox"]retain];
...
*/
	...
	NSMutableArray * UserMitLeseboxArray=[[NSMutableArray arrayWithCapacity:0]retain];
	[UserMitLeseboxArray retain];
	NSMutableDictionary* HomeDic=[[[NSMutableDictionary alloc]initWithCapacity:0]autorelease];//Dic für Volume
	...
	if ([volumesArray count]) //Es sind Volumes eingeloggt
	{
		...
		{		
			...
			NSMutableDictionary* tempUserDic=[[[NSMutableDictionary alloc]initWithCapacity:0]autorelease];//Dic für Volume
...
*/
	...
	NSString* NetzPfad=@"/Network";
		...
		NSMutableArray* NetzobjekteArray=[[[NSMutableArray alloc]initWithCapacity:0]autorelease];
		...
		NSMutableArray* NetzobjekteDicArray=[[[NSMutableArray alloc]initWithCapacity:0]autorelease];
		...
		if ([NetzobjekteArray count])
		{
			...
			NSMutableDictionary* tempNetworkDic=[[[NSMutableDictionary alloc]initWithCapacity:0]autorelease];//Dic für Volume
			...
			NSMutableArray* LoginCheckArray=[[[NSMutableArray alloc]initWithCapacity:0]autorelease];
...
- (BOOL)LeseboxValidAnPfad:(NSString*)derLeseboxPfad aufSystemVolume:(BOOL)istSystemVolume
{
  ...
  if ([Filemanager fileExistsAtPath:derLeseboxPfad ])
	{
	LeseboxValid=YES;
	//NSLog(@"Lesebox da: derLeseboxPfad: %@",derLeseboxPfad);
	}//exists at LeseboxPfad
  else
  {
	  
	  //NSLog(@"Keine Lesebox da: %@",derLeseboxPfad);
	  //PList löschen
	  ...
	  int Antwort=NSRunAlertPanel(NSLocalizedString(@"Create New Lecturebox:",@"Neue Lesebox einrichten:"),LeseboxString,NSLocalizedString(@"Create",@"Anlegen"),NSLocalizedString(@"Quit",@"Beenden"),nil);
...
{
	NSMutableArray* NamenArray=[[[NSMutableArray alloc]initWithCapacity:0]autorelease];
...
- (NSArray*)UOrdnernamenArrayVonKlassenliste
{
	BOOL erfolg;
	NSString* NamenPfad;
	NSMutableArray* KlassenArray=[[[NSMutableArray alloc] initWithCapacity:0]autorelease];
	...
	[NamenDialog setMessage:NSLocalizedString(@"Where is the list of names? It must have the format 'doc', 'txt', or 'rtf'", @"Wo ist Namenliste?")];
   ...
   int NamenHit=0;
	...
	{
		NamenPfad=[[[NamenDialog URL]path]retain]; //"home"
	}		
	else
	{
		return KlassenArray;
...
- (NSDictionary*)UOrdnernamenDicVonKlassenliste
{
  ...
  [NamenDialog setCanChooseFiles:YES];
  [NamenDialog setAllowsMultipleSelection:NO];
  [NamenDialog setMessage:NSLocalizedString(@"Where is the List of Names?",@"Wo ist die Namenliste?")];
  [NamenDialog 	setCanCreateDirectories:NO];
  int NamenHit=0;
  {
	//LeseboxHit=[LeseboxDialog runModalForDirectory:DocumentsPfad file:@"Lesebox" types:nil];
...
- (IBAction)showProjektNamenListe:(NSArray*)derArray
{
 ...
 // NSLog(@"Utils                      showProjektNamenListe nach init:UProjektArray: %@",[UProjektArray description]);
...
- (BOOL)ProjektOrdnerEinrichtenAnPfad:(NSString *)derProjektPfad
{
	...
	if (OrdnereinrichtenOK)//alles OK
	{
		...
		NSMutableArray* tempProjektNamenliste=[[[NSMutableArray alloc]initWithCapacity:0]autorelease];
		NSAlert *Warnung = [[[NSAlert alloc] init] autorelease];
		BOOL Archivleer=YES;
		...
		[Warnung addButtonWithTitle:NSLocalizedString(@"List of Names",@"Taste: Aus Namenliste")];
		[Warnung addButtonWithTitle:NSLocalizedString(@"Particular Names",@"Taste: Einzelne Namen")];
		...
		NSString* s2=NSLocalizedString(@"The list of names must be in the format 'doc', 'text', or 'rtf'",@"Format Namenliste doc, txt oder rtf");
		NSString* InformationString=[NSString stringWithFormat:@"%@\n%@",s1,s2];
		[Warnung setInformativeText:InformationString];
		[Warnung setAlertStyle:NSWarningAlertStyle];
	...
	if ([UProjektArray count])
	  {
	  	...
	  	NSMutableArray* tempNamenListeArray=[[[NSMutableArray alloc]initWithCapacity:0]autorelease];
...
- (void)UNamenEntfernenAktion:(NSNotification*)note
{
	...
	if ([[note userInfo]objectForKey:@"namen"])
	{
		...
		NSMutableArray* EntfernenPfadArray=[[[NSMutableArray alloc]initWithCapacity:0]autorelease];
		...
		if (trashURL) 
		  {
            ...
            if (![mgr moveItemAtURL:[NSURL fileURLWithPath:filePath]  toURL:[NSURL fileURLWithPath:[(NSString*)trashPath stringByAppendingPathComponent:[filePath lastPathComponent]]] error:nil])	
            {
...
- (void)UNamenEinsetzenAktion:(NSNotification*)note
{
	...
	NSMutableArray* tempPfadArray=[[[NSMutableArray alloc]initWithCapacity:0]autorelease];
	NSMutableArray* doppelteNamenArray=[[[NSMutableArray alloc]initWithCapacity:0]autorelease];
	NSFileManager *Filemanager=[NSFileManager defaultManager];
	NSMutableArray* tempNeueNamenArray=[[[NSMutableArray alloc]initWithCapacity:0]autorelease];
	...
	NSMutableArray* fehlendeOrdnerArray=[[[NSMutableArray alloc]initWithCapacity:0]autorelease];
...
//	NSLog(@"in createKommentarFuerLeser: tempKommentarOrdnerPfad %@",tempKommentarOrdnerPfad);
	...
	//Kopfstring aufbauen
	tempKopfString=[[NSString stringWithString:derLeser]retain];
...
//	NSLog(@"createKommentar	tempKommentarViewString : %@",tempKommentarViewString);
	...
	NSMutableDictionary* AufnahmeAttribute=[[[NSMutableDictionary alloc]initWithCapacity:0]autorelease];
