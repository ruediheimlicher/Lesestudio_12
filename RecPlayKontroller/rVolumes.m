				...
				switch (LeseboxOrt)
				{
					case 0://keine Lesebox
					{
						...
						if ([tempUserDic objectForKey:@"netzvolumepfad"])
						{
							...
							NSString* tempVolumeLeseboxPfad=[tempVolumeDocumentPfad stringByAppendingPathComponent:NSLocalizedString(@"Lecturebox",@"Lesebox")];
...
- (void)setNetworkArray:(NSArray*) derNetworkArray
{
	...
	if ([derNetworkArray count])
	{
		...
		while (einObjekt=[enumerator nextObject])
		{
			NSMutableDictionary* tempNetworkDic=[[[NSMutableDictionary alloc]initWithCapacity:0]autorelease];
...
- (IBAction)reportAnmelden:(id)sender
{
	...
	if ([NetworkTable numberOfSelectedRows])
	{
		...
		NSData* ComputerData = [[NSMutableData data] retain];
...
{
	NSFileManager *Filemanager=[NSFileManager defaultManager];
	LeseboxPfad=[[NSString string]retain];
